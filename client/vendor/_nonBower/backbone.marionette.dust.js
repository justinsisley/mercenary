(function (root, factory) {
  if (typeof exports === 'object') {
    ...
    // IMPORTANT: Must pre-load templates into the DustJS cache
    // so that the dust.render callback is called immediately.
    // DustJS has its own cache so the Marionette cache is not
    // required. Templates can be pre-compiled and loaded as a
    ...
    // the file system or database. If on the browser, onLoad
    // could load a template via AJAX (although would result in
    // a very slow interface before all templates had been cached).
